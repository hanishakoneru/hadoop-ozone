...
public class TestOzoneManagerBootstrap {
  ...
  /**
   * Add 4 new OMs and stop the existing 3 OMs. Verify that one of the new OMs
   * must becomes the leader.
   */
  @Test
  public void testLeaderChangeToNewOM() throws Exception {
    List<String> newOMNodeIds = testBootstrapOMs(4);

    // Stop old OMs
    for (int i = 0; i < NUM_INITIAL_OMS; i++) {
